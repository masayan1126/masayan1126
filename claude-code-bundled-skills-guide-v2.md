...
`skill-creator` スキルは、7段階のワークフロー（Decide → Write → Create test → Evaluate → Iterate → Expand test set → Optimize description）でスキル開発をガイドし、評価用の `evals/evals.json` の作成やベンチマーク実行まで自動化する。
